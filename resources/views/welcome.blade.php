...
<div x-cloak
     ...
     @modal.window="qrCodeData=$event.detail;show=!show"
        ...
        <div class="mt-8 bg-white blue-card-app dark:blue-card-app overflow-hidden border border-gray-200 rounded-lg">
            ...
            @foreach($sessions as $key => $session)
                ...
                <div class="flex w-full h-auto p-5">
                    ...
                    <div class="w-auto px-6 text-white">
                        <a title="Résultats" href="{{route('session.show-result',['slug'=>$session->slug])}}">
                            @svg('icon-fontawesome.svgs.solid.chart-simple','fill-current h6 w-6 text-gray-200')
                        </a>
                    </div>
                </div>
                {{--End item--}}
