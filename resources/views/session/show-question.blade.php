...
<div class="w-screen px-8 text-gray-200">
    <h1 class="text-gray-200 text-xl my-8">
        {{ $question->question }}
    </h1>

    {{--  --}}

    <form method="POST" action="{{route('answer.store',['session_id' => $session->id,'participant_id' =>$participant->id,'question_id' => $question->id, 'nextQuestionIndex' => $nextQuestionIndex])}}" name="answer">
        @csrf
        <div class="grid grid-cols-2 gap-4 mt-2">
            @foreach($question->purposes as $key => $purpose)
                <button
                    ...
                    class=" grow text-center py-2 rounded text-gray-200 flex justify-center items-center border-fuchsia-500">
        ...
        <textarea class="mt-8 resize-none w-full h-full blue-card-app pl-5 pt-5 pr-5 pb-5"
                  style="padding: 5px;"
                  rows="6"
                  name="comment"
